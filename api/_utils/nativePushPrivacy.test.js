...
test("native lock-screen payloads use generic text and an allowlisted route", () => {
  assert.match(source, /data: \{ route: allowlistedNativeRoute\(payload\.url\), \.\.\.\(typeof householdId/);
  ...
  assert.doesNotMatch(source, /payload\.(?:merchant|billName|accountName|amount|balance)/);
});
...
test("the unified queue considers either web or native destinations", () => {
  ...
  assert.match(source, /from\("household_members"\)/, "delivery must revalidate current household membership");
...
test("Expo tickets use a durable receipt ledger with aggregate-safe event reconciliation", () => {
  ...
  assert.match(source, /if \(!receipt\)/, "not-ready receipts must remain pending");
  assert.match(source, /DeviceNotRegistered/);
  ...
  const migration = fs.readFileSync(path.join(__dirname, "../../supabase/migrations/20260821222611_native_billing_plaid_push.sql"), "utf8");
  ...
  assert.match(migration, /create or replace function private\.apply_native_push_receipt/);
  assert.match(migration, /where id = any\(v_receipt\.event_ids\) and delivered_at is null/);
...
test("receipt persistence errors fail the worker instead of reporting success", () => {
  assert.throws(() => assertDbResult({ error: new Error("database unavailable") }, "Receipt state was not saved."), /Receipt state was not saved/);
  assert.deepEqual(assertDbResult({ data: true, error: null }, "unused"), { data: true, error: null });
});
...
test("transaction queues require a household and every sync callsite supplies it", () => {
  ...
  assert.match(source, /if \(!householdId\) throw new Error\("PUSH_HOUSEHOLD_REQUIRED"\)/);
  assert.match(syncSource, /queuePendingTransactionNotifications\(userId, item\.household_id,/);
  assert.match(syncSource, /queuePostedTransactionNotifications\(userId, item\.household_id,/);
...
test("native registration and delivery are deployment-environment scoped", () => {
  const subscription = fs.readFileSync(path.join(__dirname, "notificationRoutes/subscription.js"), "utf8");
  assert.match(subscription, /environment !== pushEnvironment\(\)/);
  assert.match(subscription, /PUSH_ENVIRONMENT_MISMATCH/);
  assert.match(source, /native_push_devices"\)\.select\("id,expo_push_token"\)[\s\S]{0,180}\.eq\("environment", pushEnvironment\(\)\)/);
  assert.match(source, /native_push_devices"\)\.select\("id"\)[\s\S]{0,180}\.eq\("environment", pushEnvironment\(\)\)/);
});

test("web notification tests preserve the active household and reject missing scope clearly", () => {
  const webAdapter = fs.readFileSync(path.join(__dirname, "../../artifacts/mobile/lib/pushNotifications.web.ts"), "utf8");
  const testRoute = fs.readFileSync(path.join(__dirname, "notificationRoutes/test.js"), "utf8");
  const preferencesRoute = fs.readFileSync(path.join(__dirname, "notificationRoutes/preferences.js"), "utf8");
  const subscriptionRoute = fs.readFileSync(path.join(__dirname, "notificationRoutes/subscription.js"), "utf8");
  assert.match(webAdapter, /JSON\.stringify\(\{ type, householdId \}\)/);
  assert.match(webAdapter, /notifications\/preferences[\s\S]{0,160}cache: "no-store"/);
