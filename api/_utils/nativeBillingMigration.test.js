...
test("native billing tables are service-only and replay-safe", () => {
  for (const table of ["billing_purchase_intents", "billing_purchase_bindings", "billing_entitlements", "billing_events", "billing_sandbox_testers", "native_push_devices", "native_push_receipts"]) {
  ...
  assert.match(migration, /where public\.billing_events\.processing_status = 'retryable'/i);
...
test("service wrappers keep privileged billing logic private and transfers revoke without rebinding", () => {
  ...
  assert.match(migration, /function public\.apply_revenuecat_billing_event[\s\S]*security invoker/i);
  ...
  assert.match(migration, /function private\.apply_revenuecat_billing_transfer/i);
...
test("billing and device cascade keys have supporting indexes", () => {
  for (const name of ["billing_purchase_intents_household_fk_idx", "billing_purchase_intents_app_user_fk_idx", "billing_purchase_bindings_household_fk_idx", "billing_purchase_bindings_app_user_fk_idx", "billing_purchase_bindings_intent_fk_idx", "billing_entitlements_purchaser_fk_idx", "billing_entitlements_app_user_fk_idx", "native_push_devices_household_fk_idx"]) {
...
test("the atomic event function binds purchaser, product, intent and household", () => {
  ...
  assert.match(migration, /Purchaser identity does not own the original transaction binding/i);
