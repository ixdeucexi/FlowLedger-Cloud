...
module.exports = async function plaidStatus(req, res) {
  ...
  try {
    ...
    const items = await client.from("plaid_items").select("id,plaid_item_id,institution_id,institution_name,status,error_code,last_attempted_sync_at,last_successful_sync_at,created_at,updated_at").eq("user_id", auth.user.id).neq("status", "removed").order("created_at", { ascending: false });
