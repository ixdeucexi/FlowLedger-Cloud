...
create index if not exists subscription_bill_links_bill_idx
  on public.subscription_bill_links (bill_id);

alter table public.subscription_bill_links enable row level security;

drop policy if exists "subscription bill links: household members read"
  on public.subscription_bill_links;
create policy "subscription bill links: household members read"
  on public.subscription_bill_links
  for select to authenticated
  using ((select private.is_household_member(household_id)));

drop policy if exists "subscription bill links: household editors insert"
  on public.subscription_bill_links;
create policy "subscription bill links: household editors insert"
  on public.subscription_bill_links
...
drop policy if exists "subscription bill links: household editors update"
  on public.subscription_bill_links;
create policy "subscription bill links: household editors update"
  on public.subscription_bill_links
...
drop policy if exists "subscription bill links: household editors delete"
  on public.subscription_bill_links;
create policy "subscription bill links: household editors delete"
  on public.subscription_bill_links
...
drop trigger if exists validate_subscription_bill_link
  on public.subscription_bill_links;
create trigger validate_subscription_bill_link
before insert or update on public.subscription_bill_links
for each row execute function private.validate_subscription_bill_link();

revoke all on public.subscription_bill_links from public, anon;
grant select, insert, update, delete on public.subscription_bill_links
  to authenticated, service_role;

comment on table public.subscription_bill_links is
  'Household-scoped recurring merchant links used to prefer an existing bill during posted transaction review.';
