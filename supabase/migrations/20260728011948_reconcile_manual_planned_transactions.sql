-- Allow a posted bank transaction to replace a manually planned calendar
-- transaction. The manual row is retained as an audit record but hidden while
-- matched, so undo can restore it exactly.
...
alter table public.transactions add constraint transactions_review_resolution_check
  check (review_resolution is null or review_resolution in ('bill', 'income', 'goal', 'decision', 'snowball', 'manual', 'category', 'transfer'));
...
alter table public.transactions add constraint transactions_linked_plan_type_check
  check (linked_plan_type is null or linked_plan_type in ('goal', 'decision', 'snowball', 'transaction'));
...
alter table public.transaction_reconciliations add constraint transaction_reconciliations_resolution_check
  check (resolution in ('bill', 'income', 'goal', 'decision', 'snowball', 'manual', 'category', 'transfer'));
...
revoke all on function private.reconcile_manual_transaction(text, text, date, numeric, text) from public, anon;
revoke all on function private.undo_manual_transaction_reconciliation(text) from public, anon;
grant execute on function private.reconcile_manual_transaction(text, text, date, numeric, text) to authenticated, service_role;
grant execute on function private.undo_manual_transaction_reconciliation(text) to authenticated, service_role;

create or replace function public.reconcile_manual_transaction(
  p_transaction_id text,
  p_manual_transaction_id text,
  p_occurrence_date date,
  p_planned_amount numeric,
...
set search_path = ''
as $$
  select private.reconcile_manual_transaction(
    p_transaction_id,
...
create or replace function public.undo_manual_transaction_reconciliation(p_transaction_id text)
returns jsonb
language sql
volatile
security invoker
set search_path = ''
as $$ select private.undo_manual_transaction_reconciliation(p_transaction_id) $$;

revoke all on function public.reconcile_manual_transaction(text, text, date, numeric, text) from public, anon;
revoke all on function public.undo_manual_transaction_reconciliation(text) from public, anon;
grant execute on function public.reconcile_manual_transaction(text, text, date, numeric, text) to authenticated, service_role;
grant execute on function public.undo_manual_transaction_reconciliation(text) to authenticated, service_role;
