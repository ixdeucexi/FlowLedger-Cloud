...
export default function DeleteAccountScreen() {
  ...
  useEffect(() => {
    if (!session?.access_token || usesPassword) return;
    void AsyncStorage.getItem(OAUTH_REAUTH_STARTED_KEY).then(value => {
      const startedAt = Number(value);
      if (Number.isFinite(startedAt) && Date.now() - startedAt <= 10 * 60 * 1000) {
        setOauthAccessToken(session.access_token);
      }
    });
  }, [session?.access_token, usesPassword]);

  const reauthenticateOauth = async () => {
    if (!oauthProvider) return;
    setBusy(true);
    setError(null);
    await AsyncStorage.setItem(OAUTH_REAUTH_STARTED_KEY, String(Date.now()));
    const reauthError = oauthProvider === "apple"
      ? await signInWithApple(false)
      : await signInWithGoogle(false);
    if (reauthError) {
      await AsyncStorage.removeItem(OAUTH_REAUTH_STARTED_KEY).catch(() => undefined);
  ...
  const deleteAccount = async () => {
    if (!user || !session || demoMode || confirmation.trim() !== "DELETE") return;
    setBusy(true);
    setError(null);
    try {
      ...
      if (usesPassword) {
        ...
        if (signInError || !data.session) throw new Error("That password could not verify your identity.");
      ...
      Alert.alert(
        ...
        [{ text: "Finish", onPress: () => router.replace({ pathname: "/login", params: { deleted: "true", receipt: receipt.receiptId } } as never) }],
        { cancelable: false },
      );
    } catch (nextError) {
      ...
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 28 }]}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <Text style={[styles.body, { color: colors.mutedForeground }]}>
            {signedIn
              ? "This disconnects banks, removes your financial records, Flo history, notification registrations, simulations, preferences, and account access. A private receipt is retained for support."
          ...
          {signedIn ? (
            <View style={styles.form}>
              <View style={[styles.warning, { borderColor: `${colors.warning}55`, backgroundColor: `${colors.warning}10` }]}>
                ...
                <Text style={[styles.warningText, { color: colors.foreground }]}>If you own a household with other members, transfer ownership or remove them first. FlowLedger stops rather than deleting their shared plan.</Text>
              </View>
              {usesPassword ? (
                <>
        ...
        <Pressable accessibilityRole="link" onPress={() => void Linking.openURL(requestUrl)}><Text style={[styles.support, { color: colors.primary }]}>Need help? Email account-deletion support</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  support: { marginTop: 22, textAlign: "center", fontSize: 13, fontFamily: "Inter_700Bold" }, note: { marginTop: 14, fontSize: 12, lineHeight: 18, textAlign: "center", fontFamily: "Inter_500Medium" },
});
