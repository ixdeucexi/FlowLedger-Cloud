...
function subscriptionStatusToDecision(
  status: SubscriptionDecisionRow["status"],
...
function subscriptionDecisionToStatus(
  decision: SubscriptionDecision,
): SubscriptionDecisionRow["status"] {
...
function isSubscriptionStyleBill(bill: {
  name: string;
...
export default function MoreScreen({
  ...
  const forecastSafetyLayout = useMemo(
    () => getForecastSafetyLayout(viewportWidth),
    [viewportWidth],
  );
  ...
  const [accountMode, setAccountMode] = useState<"add" | "edit" | "reconcile">(
    "add",
  );
  ...
  const [safetyFloorText, setSafetyFloorText] = useState(
    settings.safety_floor.toString(),
  );
  const [forecastHorizonText, setForecastHorizonText] = useState(
    settings.forecast_horizon_months.toString(),
  );
  ...
  const requestedReviewTransactionId = Array.isArray(
    ...
    ? routeParams.reviewTransactionId[0]
    : routeParams.reviewTransactionId;
  const requestedReviewFilterParam = Array.isArray(routeParams.reviewFilter)
    ? routeParams.reviewFilter[0]
    : routeParams.reviewFilter;
  const requestedReviewFilter =
    requestedReviewFilterParam === "income" ||
    requestedReviewFilterParam === "expense"
  ...
  const [householdInviteRole, setHouseholdInviteRole] =
    useState<HouseholdInviteRole>("editor");
  ...
  const [managedHouseholdMember, setManagedHouseholdMember] = useState<{
    userId: string;
    label: string;
    role: HouseholdRole;
  } | null>(null);
  ...
  const [backupExported, setBackupExported] = useState(() => {
    try {
      return (
        Platform.OS === "web" &&
        globalThis.localStorage?.getItem(BACKUP_COMPLETE_KEY) === "true"
      );
    } catch {
      return false;
    }
  ...
  const inviteRoles = useMemo(
    () => householdInviteRolesFor(activeHousehold?.role),
    [activeHousehold?.role],
  );
  const subscriptionDecisionStorageKey = user?.id
    ? `flowledger_subscription_decisions_${user.id}`
    : "flowledger_subscription_decisions_guest";
  const latestIncomeChange = useMemo(
    () =>
      incomes
        ...
        .filter(
          (
            ...
            change: NonNullable<ReturnType<typeof getLatestIncomeChange>>;
            income: IncomeItem;
          } => Boolean(entry.change),
        )
        .sort((left, right) =>
          right.change.effectiveFrom.localeCompare(left.change.effectiveFrom),
        )[0] ?? null,
    [incomes],
  );
  const latestRaise =
    latestIncomeChange && latestIncomeChange.change.difference > 0.005
      ? latestIncomeChange
      : null;
  const snowballDebtTarget = useMemo(
    () =>
  ...
  useEffect(() => {
    ...
    void AsyncStorage.getItem(payRaiseNoticeKey)
      .then((seen) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!cancelled)
        setSubscriptionDecisions(
          value
            ? normalizeStorageMap<SubscriptionDecision>(JSON.parse(value))
            : {},
        );
      ...
      const mapped = (data as SubscriptionDecisionRow[]).reduce<
        Record<string, SubscriptionDecision>
      >((acc, row) => {
  ...
  const loadMyFeedback = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .limit(25);
      if (error) throw error;
      const requestedFeedbackId = Array.isArray(routeParams.feedback)
        ? routeParams.feedback[0]
        : routeParams.feedback;
      const rows = (data ?? []) as AppFeedbackRow[];
      setMyFeedback(
        requestedFeedbackId
          ? [...rows].sort(
              (a, b) =>
                Number(b.id === requestedFeedbackId) -
                Number(a.id === requestedFeedbackId),
            )
          : rows,
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (activeSettingsSection === "admin" && feedbackAdmin)
  ...
  const saveSafetySettings = () => {
    ...
    const horizon = Math.min(
      24,
      Math.max(1, Math.round(parseFloat(forecastHorizonText) || 6)),
    );
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      setFeedbackType("bug");
      setFeedbackNotice(
        "Thank you for your feedback. It’s appreciated and will be reviewed.",
      );
  ...
  const buildCodexFeedbackPrompt = (item: AppFeedbackRow) => {
    const typeLabel =
      FEEDBACK_TYPES.find((type) => type.id === item.feedback_type)?.label ??
      "Feedback";
    return [
  ...
  const handleCopyFeedbackForCodex = async (item: AppFeedbackRow) => {
    ...
    try {
      if (Platform.OS === "web" && globalThis.navigator?.clipboard) {
        ...
        setFeedbackNotice(
          "Codex plan prompt copied. Paste it here and we’ll turn it into an update plan.",
        );
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        return;
      }
      setFeedbackNotice(
        "Copy is not available on this device. Open this feedback and paste it into Codex manually.",
      );
    } catch {
  ...
  const handleManageFeedbackAction = (
  ...
  const handleCreateHouseholdInvite = async () => {
    ...
    try {
      ...
      setHouseholdMessage(
        `${householdRoleLabel(householdInviteRole)} invite created. Share this code with the person you want to add.`,
      );
      if (Platform.OS === "web" && globalThis.navigator?.clipboard) {
  ...
  const handleJoinHousehold = async () => {
    ...
    try {
      ...
      setHouseholdMessage(
        "Household joined. FlowLedger is now showing that household plan.",
      );
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (error) {
      const message =
  ...
  const handleUpdateHouseholdRole = async (
    memberUserId: string,
  ...
  const handleLeaveHousehold = () => {
    ...
    if (!canLeaveHousehold(activeHousehold.role)) {
      Alert.alert(
        ...
        "The household owner must make someone else the owner before leaving.",
      );
      return;
    }
    ...
    confirmAction({
      ...
      message:
        "You will lose access to this shared plan. Other household members will keep their access.",
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const message =
            error instanceof Error
              ? error.message
              : "Could not leave that household.";
          setHouseholdMessage(message);
  ...
  const growthTransactions = useMemo(
    () =>
      transactions.map((transaction) => ({
        ...
        description:
          transaction.note?.trim() || transaction.category || "Transaction",
        category: transaction.category,
        accountId: transaction.account_id,
        importHash: transaction.import_hash,
        source: transaction.import_hash
          ? ("import" as const)
  ...
  const growthInsightTransactions = useMemo(
    () =>
      growthTransactions.flatMap((transaction) => {
  ...
  const growthReportTransactions = useMemo(
    () =>
      growthInsightTransactions.filter((transaction) =>
        transaction.date.startsWith(currentMonthPrefix),
      ),
    [currentMonthPrefix, growthInsightTransactions],
  );
  const growthBills = useMemo(
    () =>
      bills.map((bill) => ({
        id: bill.id,
        name: bill.name,
    ...
    return subscriptions.filter(
      (candidate) =>
        ...
        !billNames.some(
          (name) =>
            name &&
            (name.includes(candidate.merchant) ||
              candidate.merchant.includes(name)),
        ),
    );
  ...
  const reportCategoryPlan = useMemo(
    () =>
      buildCategoryPlan(
        categories,
        bills
  ...
  const forecastReadiness = useMemo(() => {
    const wantsDebt =
      onboardingPreferences.goals.includes("pay_off_debt") ||
      bills.some((bill) => bill.is_debt);
    const wantsSavings =
      onboardingPreferences.goals.includes("grow_savings") || goals.length > 0;
    ...
    return evaluateForecastReadiness({
      ...
      hasCurrentBalance: activeAccounts.some((account) =>
        Number.isFinite(account.current_balance),
      ),
      incomes: incomes.length,
      recurringBills: bills.filter((bill) => bill.is_recurring && !bill.is_debt)
        .length,
  ...
  const smartReminders = useMemo(
    () =>
      buildSmartReminders({
        ...
        subscriptionIncreases: subscriptions.filter(
          (subscription) => subscription.priceIncrease,
        ).length,
        lowestBalance: null,
        safetyFloor: settings.safety_floor,
        goals: goalFundingPlans,
        needsReconcile: forecastReadiness.missing.includes(
          "Reconcile an account",
        ),
      }),
    [
      forecastReadiness.missing,
      goalFundingPlans,
      growthBills,
  ...
  >(
    () => ({
      ...
      setup: setupIsComplete
        ? { label: "Complete" }
        : {
            label: `${setupComplete}/${setupSteps.length} complete`,
            tone: "attention",
          },
      appearance: {
        label:
      ...
      backup: backupExported
        ? { label: "Backup saved" }
        : { label: "Not backed up", tone: "attention" },
      deleted: {
        label: deletedTransactions.length
          ? formatCountStatus(deletedTransactions.length, "item")
          : "Empty",
      },
      membership: { label: membershipStatusLabel },
      admin: newFeedbackCount
        ? {
            label: formatCountStatus(newFeedbackCount, "new item"),
            tone: "attention",
          }
        : { label: "Admin tools" },
    }),
  ...
  const saveSubscriptionDecisions = async (
    next: Record<string, SubscriptionDecision>,
  ) => {
  ...
  const markSubscriptionDecision = async (
  ...
  const handleCreateBillFromSubscription = async (
    ...
    const dueDay = Math.min(
      28,
      Math.max(1, Number(latestDate.slice(8, 10)) || 1),
    );
    confirmAction({
  ...
  const handleDeleteCategory = (name: string) => {
    const inUse =
      bills.filter((b) => b.category === name).length +
  ...
  const handleExport = async () => {
    try {
      ...
      const accountHeader =
        "Id,Name,Type,CurrentBalance,BalanceAsOf,LastReconciledAt,IsActive";
      ...
      const incomeHeader =
        "Name,Amount,Frequency,StartDate,NextPaymentDate,LastReviewedAt";
      ...
      const billHeader =
        "Name,Amount,Category,Priority,IsDebt,Balance,InterestRate,DueDay,IsRecurring,Frequency";
      ...
      const txHeader =
        "Date,Amount,Category,Note,AccountId,LinkedBillId,TransferGroupId,ImportHash";
      ...
      const ovrRows = overrides
        .map((o) =>
          [o.bill_id, o.month, o.year, o.custom_amount ?? "", o.paid_amount]
            .map(csvCell)
            .join(","),
        )
        .join("\n");
      ...
      try {
        if (Platform.OS === "web")
          globalThis.localStorage?.setItem(BACKUP_COMPLETE_KEY, "true");
      } catch {}
    } catch {
    ...
    try {
      ...
      if (!rows.length) {
        Alert.alert(
          ...
          "Use a CSV with Date, Description, and Amount columns (or separate Debit and Credit columns).",
        );
        return;
      }
      ...
      Alert.alert(
        "Import failed",
        "The statement could not be imported. Your existing transactions were not changed.",
      );
    }
  };
  ...
  const handleStatementImport = () => {
    ...
    if (!active.length) {
      Alert.alert(
        ...
        "Transactions need an account so FlowLedger can detect duplicate statement rows.",
      );
      return;
    }
    if (active.length === 1) {
  ...
  const handleResetFlo = () => {
    ...
    confirmAction({
      ...
      message:
        "Remove Flo's saved preferences and context? Your financial data will not change.",
      confirmText: "Reset",
      destructive: true,
      onConfirm: async () => {
  ...
  const handleShowInstallPrompt = () => {
    ...
    Alert.alert(
      "Install FlowLedger",
      "Open FlowLedger in your phone browser, then use Add to Home Screen.",
    );
  };
      ...
      >
          ...
          <>
            ...
            >
              ...
              >
                <FloLogo size={54} />
                <View style={{ flex: 1 }}>
                  <Text style={[styles.floSetupTitle, { color: c.foreground }]}>
                    Let&apos;s set up your plan.
                  </Text>
                  <Text
                    style={[styles.floSetupDesc, { color: c.mutedForeground }]}
                  >
                    I&apos;ll ask one question at a time, then FlowLedger will
                    use your answers for forecasts and decisions.
                  </Text>
                </View>
              </View>
              <View
                style={[
                  styles.floQuestionCard,
                  { backgroundColor: c.muted, borderColor: c.border },
                ]}
              >
                <Text style={[styles.floQuestionEyebrow, { color: c.primary }]}>
                  Flo asks
                </Text>
                <Text style={[styles.floQuestionText, { color: c.foreground }]}>
                  {currentSetupStep.label}
                </Text>
                <Text
                  style={[styles.floQuestionHelp, { color: c.mutedForeground }]}
                >
                  {currentSetupStep.detail}
                </Text>
                <Pressable
                  onPress={() => handleSetupStep(currentSetupStep.key)}
                  style={({ pressed }) => [
                    styles.floQuestionButton,
                    { backgroundColor: c.primary, opacity: pressed ? 0.8 : 1 },
                  ]}
                >
                  <Text
                    style={[
                      styles.floQuestionButtonText,
                      { color: c.primaryForeground },
                    ]}
                  >
                    {currentSetupStep.done
                      ? "Review this with Flo"
                      : currentSetupStep.action}
                  </Text>
                </Pressable>
              </View>
              <View
                style={[
                  styles.setupProgressTrack,
                ...
                <View
                  style={[
                    styles.setupProgressFill,
                    {
                      ...
                      width:
                        `${Math.round((setupComplete / setupSteps.length) * 100)}%` as any,
                    },
                  ]}
                />
              </View>
              <Text
                style={[styles.setupProgressText, { color: c.mutedForeground }]}
              >
              ...
              {setupSteps.map((step, index) => (
                <View
                  ...
                  style={[
                    styles.floSetupStep,
                    { borderTopWidth: index ? 1 : 0, borderTopColor: c.border },
                  ]}
                >
                  <View
                    style={[
                      styles.floSetupNumber,
                      {
                        backgroundColor: step.done ? c.success + "18" : c.muted,
                      },
                    ]}
                  >
                    {step.done ? (
                  ...
                  >
                    <Text
                      style={[
                        styles.floSetupActionText,
                        { color: step.done ? c.mutedForeground : c.primary },
                      ]}
                    >
          ...
          <>
            ...
            >
              <View
                style={[
                  styles.themeRow,
                  isCompactSettingsLayout(viewportWidth) &&
                    styles.themeRowCompact,
          ...
          <>
            ...
            >
              ...
              {households.length > 1 && (
                <View style={styles.householdChipWrap}>
                  {households.map((household) => {
                    const selected =
                      activeHousehold?.householdId === household.householdId;
                    return (
                      <Pressable
              ...
              >
                <View style={styles.householdPanelHeader}>
                  ...
                  <Pressable
                    ...
                    hitSlop={10}
                    style={({ pressed }) => ({
                      opacity: pressed || householdBusy ? 0.6 : 1,
                    })}
                  >
                    <Feather name="refresh-cw" size={16} color={c.primary} />
                  </Pressable>
                ...
                {householdMembers.length === 0 ? (
                  ...
                  householdMembers.map((member) => {
                    const label =
                      member.displayName ||
                      member.email ||
                      `Member ${member.userId.slice(0, 6)}`;
                    const assignableRoles = householdAssignableRolesFor(
                      activeHousehold?.role,
                      member.role,
                      member.isCurrentUser,
                    );
                    const canRemove = canRemoveHouseholdMember(
                      activeHousehold?.role,
                      member.role,
                      member.isCurrentUser,
                    );
                ...
                <Pressable
                  ...
                  style={({ pressed }) => [
                    styles.leaveHouseholdButton,
                    {
                      ...
                      opacity: householdBusy ? 0.55 : pressed ? 0.72 : 1,
                    },
                  ]}
                >
                  ...
                  <View style={styles.memberInfo}>
                    ...
                    <Text
                      style={[styles.memberMeta, { color: c.mutedForeground }]}
                    >
                      {activeHousehold.role === "owner"
                        ? "Transfer ownership before leaving."
                        : "Remove your access to this shared plan."}
                    </Text>
                  </View>
                </Pressable>
              ) : null}
              ...
              <HouseholdMemberActionsModal
                ...
                onChangeRole={(role) => {
                  ...
                  void handleUpdateHouseholdRole(
                    managedHouseholdMember.userId,
                    role,
                  ).then((saved) => {
                    if (saved)
                      setManagedHouseholdMember((current) =>
                        current ? { ...current, role } : current,
                      );
                  });
                }}
                onRemove={() => {
                  if (managedHouseholdMember)
                    return handleRemoveHouseholdMember(
                      managedHouseholdMember.userId,
                      managedHouseholdMember.label,
                    );
                }}
              />
              ...
              >
                <Text
                  style={[styles.householdPanelTitle, { color: c.foreground }]}
                >
                  Share this household
                </Text>
                <Text style={[styles.switchDesc, { color: c.mutedForeground }]}>
                  Invite someone to the active household. They’ll only see this
                  household’s plan, not code, keys, or admin tools.
                </Text>
                    ...
                    <Pressable
                      ...
                      style={({ pressed }) => [
                        styles.addBtn,
                        {
                          backgroundColor: c.primary + "18",
                          borderRadius: 10,
                          opacity: pressed || householdBusy ? 0.65 : 1,
                        },
                      ]}
                    >
              ...
              >
                ...
                <View
                  style={[
                    styles.joinRow,
                    stackCompactAccountControls && styles.joinRowCompact,
                  ]}
                >
                  <TextInput
                    style={[
                    ...
                    onChangeText={(text) =>
                      setHouseholdJoinCode(text.toUpperCase())
                    }
                  ...
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      ...
                      stackCompactAccountControls && styles.joinButtonCompact,
                      {
                        backgroundColor: c.primary,
                        opacity:
                          pressed || householdBusy || !householdJoinCode.trim()
                            ? 0.55
                            : 1,
                      },
                    ]}
              ...
              >
                <View style={styles.householdPanelHeader}>
                  ...
                  <Pressable
                    onPress={refreshHouseholdActivity}
                    disabled={householdBusy}
                    hitSlop={10}
                    style={({ pressed }) => ({
                      opacity: pressed || householdBusy ? 0.6 : 1,
              ...
              {!!householdMessage && (
                <Text
                  style={[
                    ...
                    {
                      color: /couldn|invalid|expired|required|only/i.test(
                        householdMessage,
                      )
                        ? c.destructive
                        : c.success,
            ...
            >
                ...
                style={[
                  styles.confidenceBox,
                  {
                    backgroundColor:
                      forecastConfidence.level === "high"
                        ? c.success + "14"
                        : forecastConfidence.level === "medium"
                          ? "#f59e0b18"
                          : c.destructive + "12",
                  },
                ]}
              ...
              {accounts
                ...
                .map((account, index) => {
                  const reviewed =
                    account.last_reconciled_at ?? account.balance_as_of;
                  const age = Math.max(
                    0,
                    Math.floor(
                      (Date.now() - new Date(reviewed).getTime()) / 86_400_000,
                  ...
                  return (
                    <View
                      ...
                      style={[
                        styles.accountRow,
                        stackCompactAccountControls && styles.accountRowCompact,
                        {
                          borderTopWidth: index ? 1 : 0,
                          borderTopColor: c.border,
                        },
                    ...
                    >
                      ...
                      <Pressable
                        style={{ flex: 1 }}
                        onPress={() => openAccount("edit", account)}
                      >
                        <Text
                          style={[styles.accountName, { color: c.foreground }]}
                        >
                          {account.name}
                        </Text>
                        <Text
                          style={[
                            ...
                            {
                              color:
                                age > 30 ? c.destructive : c.mutedForeground,
                            },
                          ]}
                        >
                          ...
                          {age === 0
                            ? "reconciled today"
                            : `${age} days since review`}
                        </Text>
                      </Pressable>
                      <View
                        style={[
                          styles.accountRight,
                          stackCompactAccountControls &&
                            styles.accountRightCompact,
                        ]}
                      >
                        <Text
                          style={[
                            styles.incomeMonthly,
                            { color: c.foreground },
                          ]}
                        >
                        ...
                        >
                          <Text
                            style={[styles.reconcileText, { color: c.primary }]}
                          >
                            Reconcile
                          </Text>
                        </Pressable>
        ...
        {activeSettingsSection === "money" && (
          <>
            ...
            >
              <Text
                style={[styles.planningModeIntro, { color: c.mutedForeground }]}
              >
                Choose your budgeting and payoff tools. Switching keeps your
                data.
              </Text>
              <View style={styles.planningModeList}>
                <PlanningToolToggle
                  ...
                  onPress={() => {
                    if (settings.zeroBasedBudgetEnabled)
                      void updateSettings({ zeroBasedBudgetEnabled: false });
                ...
                <PlanningToolToggle
                  ...
                  onPress={() =>
                    void updateSettings({
                      debtPayoffEnabled: !settings.debtPayoffEnabled,
                    })
                  }
                />
              </View>
              <Text
                style={[
                  styles.planningModeDescription,
                  { color: c.mutedForeground },
                ]}
              >
            ...
            >
              {incomes.length === 0 ? (
                ...
                incomes.map((item, i) => {
                  ...
                  const monthly = incomeAmountToMonthly(
                    currentAmount,
                    item.frequency,
                  );
                  const historyCount = item.amount_history?.length ?? 0;
                  const latestEffectiveFrom = item.amount_history
                    ?.slice()
                    .sort((a, b) =>
                  ...
                  return (
                    ...
                    >
                      ...
                      <Pressable
                        ...
                        style={styles.incomeInfo}
                      >
                        <Text
                          style={[styles.incomeName, { color: c.foreground }]}
                        >
                          {item.name}
                        </Text>
                        <Text
                        ...
                        >
                          ...
                          {latestEffectiveFrom
                            ? ` · current from ${formatIncomeEffectiveFrom(latestEffectiveFrom)}`
                            : item.start_date
                              ? ` · from ${formatIncomeEffectiveFrom(item.start_date)}`
                              : ""}
                        </Text>
                        {historyCount > 0 ? (
                          <View style={styles.incomeHistoryHint}>
                            ...
                            <Text
                              style={[
                                styles.incomeHistoryHintText,
                                { color: c.primary },
                              ]}
                            >
                              {historyCount} pay{" "}
                              {historyCount === 1 ? "change" : "changes"} · tap
                              for history
                            </Text>
                          </View>
                        ) : null}
                      ...
                      <View style={styles.incomeRight}>
                        <Text
                          style={[styles.incomeMonthly, { color: c.success }]}
                        >
                          ${monthly.toFixed(0)}
                          <Text
                            style={[
                              styles.incomeMonthlyUnit,
                              { color: c.mutedForeground },
                            ]}
                          >
                            /mo
              ...
              {incomes.length > 0 && (
                <View
                  style={[styles.incomeTotal, { borderTopColor: c.border }]}
                >
                  <Text
                    style={[
                      styles.incomeTotalLabel,
                      { color: c.mutedForeground },
                    ]}
                  >
                    Total Monthly Income
                  </Text>
                  <Text style={[styles.incomeTotalValue, { color: c.success }]}>
                    ${totalMonthlyIncome.toFixed(0)}/mo
                  </Text>
                </View>
              )}
              <Pressable
                ...
                >
                  ...
                  <View style={styles.switchInfo}>
                    ...
                    <Text
                      style={[styles.switchDesc, { color: c.mutedForeground }]}
                    >
                      Assign and move money in Budget.
                    </Text>
                  </View>
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      Debt priorities are auto-assigned by balance (lowest
                      balance = priority #1).
                    </Text>
                  </View>
                </View>
            ...
            >
              <View>
                <Text
                  style={[
                    styles.switchLabel,
                    { color: c.foreground, marginBottom: 2 },
                  ]}
                >
                  Forecast Safety
                </Text>
                ...
                <View style={forecastSafetyLayout.fields}>
                  <View style={forecastSafetyLayout.field}>
                    ...
                    <TextInput
                      style={[
                        styles.balanceFullInput,
                        forecastSafetyLayout.input,
                        { backgroundColor: c.muted, color: c.foreground },
                      ]}
                ...
                >
                  ...
                  >
                    Save Forecast Safety
                  </Text>
                </Pressable>
              </View>
              <View
                style={[styles.balanceDivider, { borderTopColor: c.border }]}
              >
                <Text
                  style={[
                    styles.switchLabel,
                ...
                >
                  Forecast balance source
                </Text>
                <Text style={[styles.switchDesc, { color: c.mutedForeground }]}>
                  Reconcile when FlowLedger and your bank differ.
                </Text>
              </View>
            </View>
          ...
          <>
            <PlanFeatureGate feature="connected_insights" compact>
              ...
              >
                ...
                <View
                  style={[
                    styles.growthMetricGrid,
                    shouldStackSettingsMetrics(viewportWidth) &&
                      styles.growthMetricGridCompact,
                  ]}
                >
                  <View
                    style={[styles.growthMetric, { backgroundColor: c.muted }]}
                  >
                    <Text
                      style={[styles.growthMetricValue, { color: c.primary }]}
                    >
                      $
                      {subscriptions
                  ...
                  >
                    <Text
                      style={[styles.growthMetricValue, { color: c.warning }]}
                    >
                      {
                        subscriptions.filter((item) => item.priceIncrease)
                          .length
                      }
                    </Text>
                    <Text
                  ...
                  >
                    ...
                    <View style={styles.dataBody}>
                      <Text
                        style={[
                          styles.dataLabel,
                          { color: c.foreground, textTransform: "capitalize" },
                        ]}
                        numberOfLines={1}
                      >
                      ...
                      >
                        ...
                        {subscription.cadence} • {subscription.confidence}{" "}
                        confidence
                      </Text>
                      <Text
                        style={[
                          styles.growthTinyText,
                          {
                            color: subscription.priceIncrease
                              ? c.warning
                              : c.mutedForeground,
                          },
                        ]}
                      ...
                      <View style={styles.subscriptionActionRow}>
                        ...
                        <Pressable
                          onPress={() =>
                            handleMarkSubscription(subscription, "cancelled")
                          }
                          style={({ pressed }) => [
                            styles.growthPillButton,
                            {
                    ...
                    {subscriptionBillHints.slice(0, 8).map((bill, index) => (
                      ...
                      >
                        ...
                        <View style={styles.dataBody}>
                          ...
                          >
                            ${Math.max(0, bill.amount).toFixed(2)} •{" "}
                            {recurringBillCadenceLabel(bill.frequency)} • due
                            day {bill.due_day}
                          </Text>
                          <Text
                            style={[
                          ...
                          >
                            This is already in Bills, so I won’t ask you to
                            create it again.
                          </Text>
                        </View>
                      </View>
                ...
                {!subscriptions.length && !subscriptionBillHints.length && (
                  ...
                  >
                    ...
                    >
                      I have not found subscription patterns yet. Repeated
                      monthly or weekly Activity entries will appear here, and
                      subscription-style recurring bills will show here once you
                      add them.
                    </Text>
                  </View>
                )}
          ...
          <>
            ...
            >
                  ...
                  >
                    ...
                    >
                      <Feather
                        ...
                        size={17}
                        color={
                          plan.status === "on_track" ? c.success : c.warning
                        }
                      />
                    </View>
                    <View style={styles.dataBody}>
              ...
              {!goalFundingPlans.length && (
                <Text style={[styles.emptyText, { color: c.mutedForeground }]}>
                  Add a goal to build its funding plan.
                </Text>
              )}
            </View>
          </>
              ...
              {[
                {
                  ...
                  label: "Import Bills from CSV",
                  desc: "Name, Amount, Category, Balance, Interest Rate…",
                ...
                },
                {
                  ...
                  desc: "Transactions CSV with automatic duplicate detection",
                  onPress: handleStatementImport,
                  color: c.success,
                },
                {
                  icon: "download" as const,
                  label: "Export Full Backup (CSV)",
                  desc: "Accounts, income, bills, transactions, goals, and overrides",
                  onPress: handleExport,
                  color: "#6366f1",
                },
                {
                  icon: "smartphone" as const,
                  label: "Install FlowLedger App",
                  desc: "Show Apple and Android install instructions",
                  onPress: handleShowInstallPrompt,
                  color: "#22c55e",
                },
                {
                  icon: "refresh-cw" as const,
                  label: "Reset Flo Memory",
                  desc: "Remove Flo's rolling preference summary",
              ...
              >
                ...
                <View style={styles.zeroBudgetLabCopy}>
                  <View style={styles.zeroBudgetLabTitleRow}>
                    ...
                    <View
                      style={[
                        styles.zeroBudgetLabBadge,
                        { backgroundColor: c.success + "1F" },
                      ]}
                    >
                      <Text
                        style={[
                          styles.zeroBudgetLabBadgeText,
                          { color: c.success },
                        ]}
                      >
                        ISOLATED
                  ...
                  <Text
                    style={[
                      styles.zeroBudgetLabDescription,
                      { color: c.mutedForeground },
                    ]}
                  >
              ...
              <>
                ...
                >
                  <View style={styles.feedbackHero}>
                    ...
                    <View style={styles.switchInfo}>
                      <Text
                        style={[styles.switchLabel, { color: c.foreground }]}
                      >
                        Send FlowLedger feedback
                      </Text>
                      <Text
                        style={[
                          styles.switchDesc,
                      ...
                      return (
                        ...
                        >
                          <Feather
                            ...
                            size={13}
                            color={
                              active ? c.primaryForeground : c.mutedForeground
                            }
                          />
                          <Text
                            style={[
                    ...
                    {[1, 2, 3, 4, 5].map((value) => {
                      ...
                      return (
                        <Pressable
                          key={value}
                          onPress={() =>
                            setFeedbackRating(active ? null : value)
                          }
                          style={({ pressed }) => [
                  ...
                  >
                    <View
                      style={[
                        ...
                        {
                          backgroundColor: feedbackCanContact
                            ? c.success
                            : c.muted,
                          borderColor: feedbackCanContact
                            ? c.success
                            : c.border,
                        },
                      ]}
                  ...
                  {feedbackNotice ? (
                    <Text
                      style={[
                        ...
                        {
                          color:
                            /thank|copied|deleted|review|updated|archived|restored|not planned/i.test(
                              feedbackNotice,
                            )
                              ? c.success
                              : c.destructive,
                  ...
                  <Pressable
                    ...
                    disabled={
                      feedbackSubmitting || !canSubmitFeedback(feedbackMessage)
                    }
                    style={({ pressed }) => [
                      styles.balanceSaveFullBtn,
                      {
                        backgroundColor: canSubmitFeedback(feedbackMessage)
                          ? c.primary
                          : c.muted,
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.balanceSaveBtnText,
                        {
                          color: canSubmitFeedback(feedbackMessage)
                            ? c.primaryForeground
                            : c.mutedForeground,
                        },
                      ]}
                ...
                >
                  <View style={styles.feedbackInboxHeader}>
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        Your feedback and replies stay here even when phone
                        alerts are off.
                      </Text>
                    </View>
                    <Pressable
                      ...
                      >
                        <View style={styles.feedbackInboxTop}>
                          <View style={{ flex: 1 }}>
                            <Text
                              style={[
                                styles.feedbackInboxTitle,
                                { color: c.foreground },
                              ]}
                            >
                              {FEEDBACK_TYPES.find(
                                (type) => type.id === item.feedback_type,
                              )?.label ?? "Feedback"}
                            </Text>
                            <Text
                              style={[
                                styles.feedbackInboxMeta,
                                { color: c.mutedForeground },
                              ]}
                            >
                              {formatMemberDate(item.created_at)}
                            </Text>
                          </View>
                          <View
                            style={[
                        ...
                        <Text
                          style={[
                            styles.feedbackHistoryStatus,
                            { color: c.mutedForeground },
                          ]}
                        >
                          {feedbackStatusMessage(item.status)}
                        </Text>
                        {item.admin_note ? (
                          <View
                            style={[
                              styles.feedbackReply,
                              {
                                backgroundColor: c.card,
                ...
                >
                  <View style={styles.feedbackInboxHeader}>
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        Work from Active, then archive completed decisions.
                        Tester history stays available unless you permanently
                        delete it.
                      </Text>
                    </View>
                    <Pressable
                  ...
                  <ScrollView
                    ...
                    contentContainerStyle={styles.feedbackFilterRow}
                  >
                    {(
                      [
                        "active",
                  ...
                  {feedbackInboxLoading ? (
                    <Text
                      style={[
                        styles.switchDesc,
                        { color: c.mutedForeground, marginTop: 10 },
                      ]}
                    >
                      Loading feedback...
                    </Text>
                        ...
                        <View style={styles.feedbackInboxFooter}>
                          ...
                          <View style={styles.feedbackAdminActions}>
                            <Pressable
                              ...
                              onPress={() =>
                                void handleCopyFeedbackForCodex(item)
                              }
                              style={({ pressed }) => [
                            ...
                            >
                              ...
                              <Text
                                style={[
                                  styles.feedbackAdminActionText,
                                  { color: c.primary },
                                ]}
                              >
                                Copy Codex Plan
        ...
        >
          <Pressable
            ...
            onPress={() => setZeroBudgetIntroVisible(false)}
          >
            <Pressable
              style={[
                styles.infoSheet,
            ...
            >
              <View style={styles.infoSheetHeader}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text
                    style={[styles.infoSheetTitle, { color: c.foreground }]}
                  >
              ...
              <Text
                style={[styles.infoSheetDesc, { color: c.mutedForeground }]}
              >
                Assign planned income to bills, savings, debt, and spending.
              </Text>
              <View style={styles.zeroBudgetIntroSteps}>
                {[
                  "Confirm monthly income",
                  "Review suggested assignments",
                  "Reach $0 left to assign",
                ].map((step, index) => (
                  <View key={step} style={styles.zeroBudgetIntroStep}>
                    <View
                      style={[
                        styles.zeroBudgetIntroNumber,
                        { backgroundColor: c.primary + "18" },
                      ]}
                    >
                      <Text
                        style={[
                          styles.zeroBudgetIntroNumberText,
                          { color: c.primary },
                        ]}
                      >
                        {index + 1}
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.infoDoneButton,
                  { backgroundColor: c.primary, opacity: pressed ? 0.82 : 1 },
                ]}
              >
                <Text
                  style={[styles.infoDoneText, { color: c.primaryForeground }]}
                >
                  Turn on and set up this month
                </Text>
              </Pressable>
              <Pressable
                onPress={() => setZeroBudgetIntroVisible(false)}
                style={styles.zeroBudgetIntroLater}
              >
                <Text style={[styles.switchDesc, { color: c.mutedForeground }]}>
                  Not now
                </Text>
        ...
        <PayRaiseCelebrationModal
          ...
          onViewDebt={() => {
            ...
            router.push({
              ...
              params: {
                suggested: (latestRaise?.change.difference ?? 0).toFixed(2),
              },
            } as never);
          }}
        />
        ...
        <AccountModal
          ...
          onReconcile={(balance, date) =>
            selectedAccount
              ? reconcileAccount(selectedAccount.id, balance, date)
              : Promise.resolve()
          }
        />
      </ScrollView>
...
function PlanningToolToggle({
  ...
  return (
    ...
    >
      <View
        style={[
          styles.planningModeIcon,
          { backgroundColor: enabled ? c.primary : c.card },
        ]}
      >
        <Feather
          name={icon}
      ...
      <View style={styles.planningModeCopy}>
        <Text style={[styles.planningModeTitle, { color: c.foreground }]}>
          {label}
        </Text>
        <Text style={[styles.planningModeShort, { color: c.mutedForeground }]}>
          {description}
        </Text>
      </View>
      <View
        style={[
          styles.planningToolSwitch,
          compactLayout && styles.planningToolSwitchCompact,
          { backgroundColor: enabled ? c.primary : c.border },
        ]}
      >
        <View
          style={[
            styles.planningToolThumb,
            {
...
const styles = StyleSheet.create({
  ...
  zeroBudgetLabBadgeText: {
    fontSize: 8,
    fontFamily: "Inter_800ExtraBold",
    letterSpacing: 0.7,
  },
  zeroBudgetLabDescription: {
    fontSize: 11,
    fontFamily: "Inter_500Medium",
    lineHeight: 16,
    marginTop: 3,
