...
export function AccountModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Couldn't save. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.overlay}>
        <View style={[styles.sheet, { backgroundColor: c.background }]}>
              ...
              <View style={styles.types}>
                {TYPES.map(option => (
                  <Pressable
                    ...
                    style={[styles.type, { backgroundColor: type === option.value ? c.primary : c.muted }]}
                  >
                    <Text style={[styles.typeText, { color: type === option.value ? c.primaryForeground : c.mutedForeground }]}>
                      {option.label}
                    </Text>
                  </Pressable>
                ))}
          ...
          <Pressable disabled={saving} onPress={submit} style={[styles.save, { backgroundColor: c.primary, opacity: saving ? 0.7 : 1 }]}>
            <Text style={[styles.saveText, { color: c.primaryForeground }]}>
              {saving ? "Saving..." : mode === "reconcile" ? "Confirm Reconciliation" : "Save Account"}
            </Text>
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 11, fontFamily: "Inter_600SemiBold", textTransform: "uppercase", marginTop: 14, marginBottom: 6 },
  input: { height: 48, borderWidth: 1, borderRadius: 10, paddingHorizontal: 13, fontSize: 16 },
  types: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  type: { paddingHorizontal: 12, paddingVertical: 9, borderRadius: 9 },
  typeText: { fontSize: 12, fontFamily: "Inter_600SemiBold" },
  help: { fontSize: 12, lineHeight: 18, marginTop: 10 },
  error: { fontSize: 12, lineHeight: 17, marginTop: 12, fontFamily: "Inter_700Bold" },
  save: { height: 48, borderRadius: 11, alignItems: "center", justifyContent: "center", marginTop: 18 },
