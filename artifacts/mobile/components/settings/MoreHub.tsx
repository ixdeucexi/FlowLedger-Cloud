...
export function MoreHub({
  ...
  const compactLayout = isCompactSettingsLayout(
    viewportWidth / Math.max(1, fontScale),
  );
  const stackQuickAccess = shouldStackQuickAccess(viewportWidth, fontScale);
    ...
    <>
      ...
      >
        <View style={styles.householdCopy}>
          ...
          <Text style={[styles.identity, { color: colors.mutedForeground }]}>
      ...
      <View style={styles.groupBlock}>
        ...
        <View
          style={[
            styles.quickGrid,
            stackQuickAccess && styles.quickGridStacked,
          ]}
        >
          {[
            {
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                styles.quickCard,
                !stackQuickAccess && styles.quickCardSquare,
                compactLayout && styles.quickCardCompact,
                stackQuickAccess && styles.quickCardStacked,
                {
            ...
            >
              <View
                style={[
                  styles.quickIcon,
                  compactLayout && styles.quickIconCompact,
                  { backgroundColor: colors.primary + "16" },
                ]}
              >
              ...
              <View style={styles.quickCopy}>
                <Text
                  style={[
                    styles.quickLabel,
                    compactLayout && styles.quickLabelCompact,
                    { color: colors.foreground },
                  ]}
                  numberOfLines={stackQuickAccess ? undefined : 2}
                >
                  {action.label}
                </Text>
                {!compactLayout ? (
      ...
      {visibleSettingsGroups(isAdmin).map((group) => (
        <View key={group.id} style={styles.groupBlock}>
          ...
          <Text
            style={[styles.groupDescription, { color: colors.mutedForeground }]}
          >
            {group.description}
          </Text>
          <View
            ...
            {group.sectionIds.map((sectionId, index) => {
              ...
              return (
                <Pressable
                  key={section.id}
                  nativeID={
                    section.id === "setup" ? "guided-tour-more" : undefined
                  }
                  accessibilityRole="button"
                  accessibilityLabel={
                    status ? `${section.label}, ${status.label}` : section.label
                  }
                ...
                >
                  <View
                    style={[
                      styles.rowIcon,
                      compactLayout && styles.rowIconCompact,
                      { backgroundColor: colors.primary + "11" },
                    ]}
                  >
                    <Feather
                      name={
                        section.icon as ComponentProps<typeof Feather>["name"]
                      }
                      size={19}
                  ...
                  <View
                    style={[
                      styles.rowMain,
                      compactLayout && styles.rowMainCompact,
                    ]}
                  >
