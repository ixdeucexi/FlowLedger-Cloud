...
begin
  ...
  values (p_household_id, encode(digest(invite_code, 'sha256'), 'hex'), coalesce(p_role, 'editor'), auth.uid());
  return invite_code;
end $$;
...
begin
  ...
  if clean_code = '' then raise exception 'Invite code is required'; end if;
  select * into invite
  ...
  where code_hash = encode(digest(clean_code, 'sha256'), 'hex')
    ...
    and expires_at > now()
  limit 1;
  if invite.id is null then raise exception 'Invite code is invalid or expired'; end if;
  insert into household_members (household_id, user_id, role)
...
begin
  select household_id into target_household from household_invites where id = p_invite_id;
  if target_household is null then return; end if;
  if public.household_role(target_household) <> 'owner' then raise exception 'Only the household owner can revoke invites'; end if;
  update household_invites set revoked_at = now() where id = p_invite_id;
end $$;
...
create policy "households: members read" on households for select using (public.is_household_member(id));

drop policy if exists "members: household members read" on household_members;
create policy "members: household members read" on household_members for select using (public.is_household_member(household_id));

drop policy if exists "members: owner manages" on household_members;
create policy "members: owner manages" on household_members for all using (public.household_role(household_id) = 'owner') with check (public.household_role(household_id) = 'owner');

drop policy if exists "household settings: members read" on household_settings;
create policy "household settings: members read" on household_settings for select using (public.is_household_member(household_id));
drop policy if exists "household settings: editors write" on household_settings;
create policy "household settings: editors write" on household_settings for all using (public.is_household_editor(household_id)) with check (public.is_household_editor(household_id));

drop policy if exists "household invites: owners read" on household_invites;
create policy "household invites: owners read" on household_invites for select using (public.household_role(household_id) = 'owner');
