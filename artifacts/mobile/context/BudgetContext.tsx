import type {
  ...
  CashFlow,
  DailyBalance,
} from "@/lib/financialProjectionTypes";
export type {
  Bill,
  MonthlyOverride,
  BillDateMove,
...
import {
  ...
  checkingPendingBankRows,
  normalizePendingBankRows,
  normalizePendingPlanMatchRow,
  normalizeConnectedBankRows,
  normalizeAccountRow,
  normalizeMonthlyOverrideRow,
} from "@/lib/financialProjectionInput";
export { normalizeTransactionRow } from "@/lib/financialProjectionInput";
import {
  toAccountSnapshot,
  getGoalRemainingAmount,
  hasPendingSnowballBalanceApply,
  safeLocalDateInTimeZone,
} from "@/lib/financialProjection";
import { createFinancialProjectionReader } from "@/lib/financialProjectionReader";
...
import { authoritativeFreshnessTimestamp, financialProjectionPreparationMonths, indexRecordsByMonth, reuseStructurallyEqualFinancialValue, startCancellableStageQueue } from "@/lib/financialProjectionCache";
import {
...
export function BudgetProvider({ children }: { children: React.ReactNode }) {
  ...
  // One shared read-only engine per financial revision; dated reads refresh lazily.
  const financialProjection = useMemo(
    () =>
      createFinancialProjectionReader(
        {
          settings,
          bills,
          overrides,
  ...
  // Keep performance telemetry in the app, outside the shared pure engine.
  const getDailyBalances = useCallback(
    (month: number, year: number): DailyBalance[] => {
      const started = Date.now();
      const result = financialProjection.getDailyBalances(month, year);
