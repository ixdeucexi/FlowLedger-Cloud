...
export function summarizeZeroBudget(state: ZeroBudgetLabState, selectedMonth = state.selectedMonth): ZeroBudgetMonthSummary {
  const categories = state.categories.map(category => {
    const target = monthlyTarget(category);
    const assigned = roundMoney(Math.max(0, category.assignedByMonth[selectedMonth] ?? 0));
    const spent = roundMoney(Math.max(0, category.spentByMonth[selectedMonth] ?? 0));
    const available = roundMoney(assigned - spent);
    const needed = roundMoney(Math.max(0, target - assigned));
    const status = available < -0.005
      ? "overspent"
    ...
    return {
      ...
      available,
      needed,
      progress: target > 0 ? Math.min(100, Math.round((assigned / target) * 100)) : assigned > 0 ? 100 : 0,
      status,
    } satisfies ZeroBudgetCategorySummary;
  });
  const income = roundMoney(Math.max(0, state.incomeByMonth[selectedMonth] ?? state.defaultMonthlyIncome));
  const assigned = roundMoney(categories.reduce((sum, row) => sum + row.assigned, 0));
  ...
  return {
    ...
    monthlyTargets: roundMoney(categories.reduce((sum, row) => sum + row.monthlyTarget, 0)),
    categories,
  };
}
...
export function applyZeroBudgetMoney(
  ...
  return {
    ...
    categories: state.categories.map(category => {
      ...
      if (action === "spend") {
        const spent = roundMoney((category.spentByMonth[state.selectedMonth] ?? 0) + safeAmount);
        return { ...category, spentByMonth: { ...category.spentByMonth, [state.selectedMonth]: spent } };
      }
      const current = category.assignedByMonth[state.selectedMonth] ?? 0;
      const assigned = assignmentMode === "set"
        ? safeAmount
        : assignmentMode === "subtract"
          ? Math.max(0, current - safeAmount)
          : current + safeAmount;
      return { ...category, assignedByMonth: { ...category.assignedByMonth, [state.selectedMonth]: roundMoney(assigned) } };
    }),
  };
}
...
export function moveZeroBudgetCategory(state: ZeroBudgetLabState, categoryId: string, direction: -1 | 1): ZeroBudgetLabState {
  ...
  const groupCategories = state.categories.filter(item => item.groupId === category.groupId);
  const currentIndex = groupCategories.findIndex(item => item.id === categoryId);
...
export function createZeroBudgetLabState(now = new Date()): ZeroBudgetLabState {
  ...
  return {
    ...
    categories: [
      { id: "rent", groupId: "bills", name: "Rent / Mortgage", emoji: "🏠", targetAmount: 1468, targetCadence: "by_date", dueDay: 4, assignedByMonth: assigned(1468), spentByMonth: spent(1468) },
      { id: "phone", groupId: "bills", name: "Phone & Internet", emoji: "📱", targetAmount: 154, targetCadence: "by_date", dueDay: 31, assignedByMonth: assigned(35.21), spentByMonth: spent(0) },
      { id: "utilities", groupId: "bills", name: "Utilities", emoji: "⚡", targetAmount: 480, targetCadence: "monthly", assignedByMonth: assigned(480), spentByMonth: spent(350) },
      { id: "groceries", groupId: "needs", name: "Groceries", emoji: "🛒", targetAmount: 10, targetCadence: "weekly", assignedByMonth: assigned(40), spentByMonth: spent(30) },
      { id: "transportation", groupId: "needs", name: "Transportation", emoji: "🚗", targetAmount: 20, targetCadence: "weekly", assignedByMonth: assigned(80), spentByMonth: spent(60) },
      { id: "medical", groupId: "needs", name: "Medical expenses", emoji: "🩺", targetAmount: 70, targetCadence: "monthly", assignedByMonth: assigned(70), spentByMonth: spent(0) },
      { id: "emergency", groupId: "needs", name: "Emergency fund", emoji: "🛟", targetAmount: 200, targetCadence: "monthly", assignedByMonth: assigned(100), spentByMonth: spent(0) },
      { id: "dining", groupId: "wants", name: "Dining out", emoji: "🍽️", targetAmount: 200, targetCadence: "monthly", assignedByMonth: assigned(100), spentByMonth: spent(84.02) },
...
export function normalizeZeroBudgetLabState(value: unknown, now = new Date()): ZeroBudgetLabState {
  ...
  return {
    ...
    defaultMonthlyIncome: Math.max(0, Number(candidate.defaultMonthlyIncome) || 0),
    incomeByMonth: candidate.incomeByMonth && typeof candidate.incomeByMonth === "object" ? candidate.incomeByMonth : {},
...
export async function loadZeroBudgetLabState(userId: string): Promise<ZeroBudgetLabState> {
...
export async function saveZeroBudgetLabState(userId: string, state: ZeroBudgetLabState): Promise<void> {
  await AsyncStorage.setItem(storageKey(userId), JSON.stringify(state));
}

export async function resetZeroBudgetLabState(userId: string): Promise<ZeroBudgetLabState> {
