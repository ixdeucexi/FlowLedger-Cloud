...
export async function getPushNotificationStatus(userId: string, _accessToken?: string, _householdId?: string): Promise<PushNotificationStatus> {
...
export async function restorePushNotifications(accessToken: string, userId: string, _householdId?: string) {
...
export async function sendTestPushNotification(accessToken: string, type: NotificationPreferenceKey, householdId?: string) {
  const response = await apiFetch("/api/notifications/test", {
    method: "POST",
    headers: authorization(accessToken),
    body: JSON.stringify({ type, householdId }),
  });
