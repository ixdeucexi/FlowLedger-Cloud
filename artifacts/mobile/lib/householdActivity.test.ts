...
test("recent activity collapses create/update noise for one item on one day", () => {
  const summary = summarizeHouseholdActivity([
    ...
    row({ id: "created", action: "created", createdAt: "2026-07-06T18:04:00.000Z" }),
    row({ id: "duplicate", createdAt: "2026-07-06T18:03:00.000Z" }),
  ]);

  assert.deepEqual(summary.map(item => item.id), ["updated"]);
});
...
test("recent activity preserves different actors and different days", () => {
  ...
  assert.deepEqual(summary.map(item => item.id), ["tomorrow-user-1", "today-user-1", "today-user-2"]);
});

test("duplicate invite events are summarized without erasing later-day history", () => {
  const summary = summarizeHouseholdActivity([
    row({ id: "invite-2", action: "invited", entityId: null, entityType: "household_invites", entityLabel: "viewer", createdAt: "2026-07-06T18:06:00.000Z" }),
    row({ id: "invite-1", action: "invited", entityId: null, entityType: "household_invites", entityLabel: "viewer", createdAt: "2026-07-06T18:05:00.000Z" }),
    row({ id: "invite-next-day", action: "invited", entityId: null, entityType: "household_invites", entityLabel: "viewer", createdAt: "2026-07-07T18:05:00.000Z" }),
  ]);

  assert.deepEqual(summary.map(item => item.id), ["invite-next-day", "invite-2"]);
});
...
test("database trigger never falls back to a row owner for audit attribution", () => {
  ...
  assert.doesNotMatch(migration, /coalesce\(auth\.uid\(\), nullif\(source_row->>'user_id'/);
