...
test("startup stays constant until the destination screen is ready", () => {
  ...
  assert.match(layout, /SplashScreen\.setOptions\(\{ duration: 0, fade: false \}\)/);
  assert.match(layout, /const initialAppReady = coreReady/);
  assert.match(layout, /if \(initialAppReady\) setAppReady\(true\)/);
  assert.match(layout, /const navigationReady = appReady/);
  assert.match(layout, /const readyToReveal = navigationReady && \(!privacyShielded \|\| !!privacyRefreshError\)/);
  ...
  assert.doesNotMatch(layout, /STARTUP_BRAND_FADE_MS|STARTUP_BRAND_HOLD_MS|APP_REVEAL_MS/);
  assert.doesNotMatch(layout, /Animated\.(sequence|parallel|timing)|setShowStartupOverlay|setWebExitStarted/);
  assert.match(appConfig, /image: "\.\/assets\/images\/startup_f_transparent\.png"/);
...
test("startup plan loading fails closed instead of hanging on household discovery", () => {
  ...
  assert.match(budgetContext, /withLoadTimeout\(\s*verifyCurrentHouseholdMembership[\s\S]*?8000/);
  assert.match(authContext, /withTimeout\(Linking\.getInitialURL\(\), 3000, "Initial app link"\)/);
  ...
  assert.match(layout, /if \(verificationIsFresh\) setPrivacyShielded\(false\)/);
  ...
  assert.match(layout, /withStartupTimeout\([\s\S]*?readLastAppRoute[\s\S]*?1_500/);
