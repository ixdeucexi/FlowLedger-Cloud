...
let metroPort = Number(process.env.EXPO_STATIC_BUILD_METRO_PORT || 8081);
...
async function findAvailableMetroPort() {
  for (let port = metroPort; port < metroPort + 20; port++) {
  ...
  throw new Error(
    `Could not find an available Metro port starting at ${metroPort}`,
  );
}
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  if (process.env.FLOWLEDGER_REUSE_METRO === "1") {
...
async function downloadBundle(platform, timestamp) {
  ...
  const url = new URL(`${getMetroUrl("/")}${bundlePath}.bundle`);
...
function extractAssets(timestamp) {
  ...
  const extractFromBundle = (bundle, platform) => {
    for (const match of bundle.matchAll(assetPattern)) {
      ...
      const tempUrl = new URL(`${getMetroUrl()}${originalPath}`);
...
async function downloadAssets(assets, timestamp) {
  ...
  const downloadPromises = assets.map(async (asset) => {
    const tempUrl = new URL(`${getMetroUrl()}${asset.originalPath}`);
    const unstablePath = tempUrl.searchParams.get("unstable_path");
...
function updateBundleUrls(timestamp, baseUrl) {
  const updateForPlatform = (platform) => {
    ...
    bundle = bundle.replace(
      ...
      (_match, capturedPath) => {
        const tempUrl = new URL(`${getMetroUrl()}${capturedPath}`);
        const unstablePath = tempUrl.searchParams.get("unstable_path");
